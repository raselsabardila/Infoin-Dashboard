...
class UserList extends Component
{
    ...
    public function render()
    {
        $users;
        if ($this->search == null) {
            $users = User::orderBy("name","ASC")->where("role_id",1)->latest()->paginate($this->paginate);
        } else{
            $users = User::where("name",$this->search)->orWhere("name","like","%".$this->search."%")->where("role_id",1)->latest()->paginate($this->paginate);
